...
internal static class CSharpClientGenerator
{
        ...
        foreach (var (name, schema) in doc.Components.Schemas.OrderBy(kv => kv.Key))
        {
            ...
            if (metadata.IsSemanticKeyUnion(name))
            {
                var branches = metadata.GetSemanticKeyUnionBranches(name)!
                    .Select(SanitizeTypeName).ToList();
    ...
    private static void GenerateDomainStruct(StringBuilder sb, string typeName, IOpenApiSchema schema, OpenApiDocument doc)
    {
        ...
        if (isString)
        {
            ...
            if (pattern != null)
                constraintArgs.Add($"pattern: \"{SafeEmit.SafeCSharpStringLiteral(pattern)}\"");
            if (minLength != null)
        ...
        if (isString)
        {
            ...
            if (pattern != null)
                checkArgs.Add($"pattern: \"{SafeEmit.SafeCSharpStringLiteral(pattern)}\"");
            if (minLength != null)
    ...
    private static string GenerateSpecHash(string specHash)
    {
        ...
        sb.AppendLine("    /// </summary>");
        sb.AppendLine($"    public const string SpecHash = \"{SafeEmit.SafeCSharpStringLiteral(specHash)}\";");
        sb.AppendLine("}");
        return sb.ToString();
    }
    ...
    private static void GenerateMethod(StringBuilder sb, OperationMeta op, Dictionary<string, List<string>> operationExamples)
    {
        ...
        var pathExpr = SafeEmit.SafeCSharpStringLiteral(op.Path);
        foreach (var p in op.PathParams)
        {
        ...
        if (op.QueryParams.Count > 0)
        {
            ...
            foreach (var q in op.QueryParams)
            {
                ...
                if (q.Required)
                    sb.AppendLine($"        queryParts.Add(\"{SafeEmit.SafeCSharpStringLiteral(q.Name)}=\" + Uri.EscapeDataString(FormatParam({varName})));");
                else
                    sb.AppendLine($"        if ({varName} != null) queryParts.Add(\"{SafeEmit.SafeCSharpStringLiteral(q.Name)}=\" + Uri.EscapeDataString(FormatParam({varName})));");
            }
        ...
        var safeOpId = SafeEmit.SafeCSharpStringLiteral(op.OriginalOperationId);
        ...
        if (op.IsMultipart)
        {
            ...
            sb.AppendLine($"        {returnKw}await InvokeWithRetryAsync(() => SendMultipartAsync<{op.ResponseTypeName}>(path, content, ct), \"{safeOpId}\", {op.IsExemptFromBackpressure.ToString().ToLowerInvariant()}, ct);");
        }
        else if (op.IsVoidResponse)
        {
            sb.AppendLine($"        await InvokeWithRetryAsync(async () => {{ await SendVoidAsync({httpMethod}, path, {bodyArg}, ct); return 0; }}, \"{safeOpId}\", {op.IsExemptFromBackpressure.ToString().ToLowerInvariant()}, ct);");
        }
        else if (op.IsBinaryResponse)
        {
            sb.AppendLine($"        return await InvokeWithRetryAsync(() => SendBinaryAsync({httpMethod}, path, {bodyArg}, ct), \"{safeOpId}\", {op.IsExemptFromBackpressure.ToString().ToLowerInvariant()}, ct);");
        }
        else
        {
            var returnKw = op.IsEventuallyConsistent ? "return " : "return ";
            sb.AppendLine($"        {returnKw}await InvokeWithRetryAsync(() => SendAsync<{op.ResponseTypeName}>({httpMethod}, path, {bodyArg}, ct), \"{safeOpId}\", {op.IsExemptFromBackpressure.ToString().ToLowerInvariant()}, ct);");
        }
