...
internal static class Program
{
    private static int Main(string[] args)
    {
        ...
        if (repoRoot == null)
        {
            Console.Error.WriteLine("[generator] Cannot find repository root (looked for Camunda.Client.sln)");
            return 1;
        }

        var specPath = Path.Combine(repoRoot, "external-spec", "bundled", "rest-api.bundle.json");
        var outputDir = Path.Combine(repoRoot, "src", "Camunda.Client", "Generated");
    ...
    private static string? FindRepoRoot()
    {
        ...
        while (dir != null)
        {
            if (File.Exists(Path.Combine(dir, "Camunda.Client.sln")))
                return dir;
