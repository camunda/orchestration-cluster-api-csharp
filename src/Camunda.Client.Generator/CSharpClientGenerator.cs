...
internal static class CSharpClientGenerator
{
    ...
    public static void Generate(string specPath, string outputDir)
    {
        ...
        var modelsCode = GenerateModels(doc, inlineSchemas);
    ...
    private static string GenerateModels(OpenApiDocument doc, Dictionary<string, OpenApiSchema> inlineSchemas)
    {
        ...
        foreach (var (name, schema) in doc.Components.Schemas.OrderBy(kv => kv.Key))
        {
            ...
            // Check if this class has an optional tenantId property
            var tenantIdInfo = GetOptionalTenantIdInfo(schema);
        ...
        {
            GenerateClass(sb, typeName, schema, doc);
        }

        return sb.ToString();
    }

    private static void GenerateClass(StringBuilder sb, string typeName, OpenApiSchema schema, OpenApiDocument? doc = null)
    {
        var tenantIdInfo = GetOptionalTenantIdInfo(schema);
