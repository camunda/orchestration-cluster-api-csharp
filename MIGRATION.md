...
### Identifier parameters are now semantic key types

55 client method parameters moved from `string` to semantic key structs. This is the
...
|---|---|
| `GroupId` (17) | `AssignClientToGroupAsync`, `AssignGroupToTenantAsync`, `AssignMappingRuleToGroupAsync`, `AssignRoleToGroupAsync`, `AssignUserToGroupAsync`, `DeleteGroupAsync`, `GetGroupAsync`, `SearchClientsForGroupAsync`, `SearchMappingRulesForGroupAsync`, `SearchRolesForGroupAsync`, `SearchUsersForGroupAsync`, `UnassignClientFromGroupAsync`, `UnassignGroupFromTenantAsync`, `UnassignMappingRuleFromGroupAsync`, `UnassignRoleFromGroupAsync`, `UnassignUserFromGroupAsync`, `UpdateGroupAsync` |
| `RoleId` (17) | `AssignRoleToClientAsync`, `AssignRoleToGroupAsync`, `AssignRoleToMappingRuleAsync`, `AssignRoleToTenantAsync`, `AssignRoleToUserAsync`, `DeleteRoleAsync`, `GetRoleAsync`, `SearchClientsForRoleAsync`, `SearchGroupsForRoleAsync`, `SearchMappingRulesForRoleAsync`, `SearchUsersForRoleAsync`, `UnassignRoleFromClientAsync`, `UnassignRoleFromGroupAsync`, `UnassignRoleFromMappingRuleAsync`, `UnassignRoleFromTenantAsync`, `UnassignRoleFromUserAsync`, `UpdateRoleAsync` |
| `MappingRuleId` (9) | `AssignMappingRuleToGroupAsync`, `AssignMappingRuleToTenantAsync`, `AssignRoleToMappingRuleAsync`, `DeleteMappingRuleAsync`, `GetMappingRuleAsync`, `UnassignMappingRuleFromGroupAsync`, `UnassignMappingRuleFromTenantAsync`, `UnassignRoleFromMappingRuleAsync`, `UpdateMappingRuleAsync` |
| `ClientId` (6) | `AssignClientToGroupAsync`, `AssignClientToTenantAsync`, `AssignRoleToClientAsync`, `UnassignClientFromGroupAsync`, `UnassignClientFromTenantAsync`, `UnassignRoleFromClientAsync` |
| `ClusterVariableName` (6) | `DeleteGlobalClusterVariableAsync`, `DeleteTenantClusterVariableAsync`, `GetGlobalClusterVariableAsync`, `GetTenantClusterVariableAsync`, `UpdateGlobalClusterVariableAsync`, `UpdateTenantClusterVariableAsync` |

`TenantId` and `Username` parameters are **not** in this list — they were already
semantic keys in v9 and are unchanged.

The compiler finds every affected call site for you: each one fails with
`CS1503: cannot convert from 'string' to 'GroupId'`.
...
- **Branded types**: All existing `ICamundaKey` types (`ProcessDefinitionKey`, `UserTaskKey`, etc.) retain the same API, and `TenantId` / `Username` parameters are unchanged. Note that v10 *adds* the branded types `GroupId`, `RoleId`, `ClientId`, `MappingRuleId`, `AgentInstanceKey` and `ClusterVariableName`, and applies several of them to existing method parameters — see [Identifier parameters are now semantic key types](#identifier-parameters-are-now-semantic-key-types).
